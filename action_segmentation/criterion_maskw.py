...
class SetCriterion_maskw(nn.Module):
    @configurable
    ...
    def from_config(cls, cfg):
        ...
        weight_dict = {"loss_ce": 1, "loss_mask": mask_weight, "loss_dice": dice_weight} # for combine all the loss
        losses = ['labels', 'masks']
        return {
    ...
    def loss_masks(self, outputs, targets, indices, num_masks):
        ...
        pos_weight = target_masks * 10
        bce_loss = torch.nn.BCEWithLogitsLoss(pos_weight=pos_weight, reduction='none')
        loss_mask = bce_loss(src_masks, target_masks)

        losses = {
            "loss_mask": loss_mask.mean(1).sum() / num_masks , #sigmoid_focal_loss(src_masks, target_masks, num_masks), # + F1_loss , ##use the matched pair to get loss, which is also used for cost in processing of matching
        ...
        pos_weight = target_boundarys * 100
    ...
    def get_loss(self, loss, outputs, targets, indices, num_masks):  ## loss ："labels" or "masks"
        loss_map = {"labels": self.loss_labels, "masks": self.loss_masks}  ## just find the loss functions here
