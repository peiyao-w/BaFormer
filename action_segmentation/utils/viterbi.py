...
class PoissonModel(LengthModel):
    def __init__(self, model, num_classes, sample_rate, max_length=2000, renormalize=True): # max_length=20000
...
class Viterbi(object):
    ...
    def frame_score(self, frame_scores, t, label):
        if t >= self.frame_sampling:
            ...
            # return frame_scores[t, int(label.item())] - frame_scores[t - self.frame_sampling, int(label.item())]
        else:
            return frame_scores[t, label]
            # return frame_scores[t, int(label.item())]
