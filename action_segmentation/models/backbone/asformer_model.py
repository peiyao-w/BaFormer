...
class ASFormerEncoder(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        for layer in self.layers:
            ...
            inter_fea.append(feature)

        out = self.conv_out(feature) * mask[:, 0:1, :]
        outputs = {'frame_wise_out': out,
                    'feature': feature,
                   'inter_feature': inter_fea}
        return outputs
...
class Decoder_fea(nn.Module):
    def __init__(self, num_layers, r1, r2, num_f_maps, input_dim, num_classes, att_type, alpha):
        super( ).__init__()  # self.position_en = PositionalEncoding(d_model=num_f_maps)
        self.conv_1x1 = nn.Conv1d(input_dim, num_f_maps, 1)
        self.layers = nn.ModuleList(
            [AttModule(2 ** i, num_f_maps, num_f_maps, r1, r2, att_type, 'decoder', alpha) for i in  # 2 ** i
             range(num_layers)])
