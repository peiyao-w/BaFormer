...
class MaskedTransformerDecoderBD(nn.Module):
    @configurable
    def __init__(
            self,
            in_channels,
    ...
    def forward(self, x, mask_features, boundary, mask=None):
        ...
        outputs_class, outputs_mask, attn_mask = self.forward_prediction_heads(output, mask_features, boundary,attn_mask_target_size=size_list[0])
        ...
        for i in range(self.num_layers):
            ...
            outputs_class, outputs_mask, attn_mask = self.forward_prediction_heads(output, mask_features, boundary,
    ...
    def forward_prediction_heads(self, output, mask_features, boundary, attn_mask_target_size):
        ...
        attn_mask = self.process_mask(attn_mask, boundary)
        attn_mask = (attn_mask.sigmoid().unsqueeze(1).repeat(1, self.num_heads, 1, 1).flatten(0, 1) < 0.5).bool() #[nhead, q, l]each cahnnal differ
        attn_mask = attn_mask.detach()
        # attn_mask = torch.zeros_like(attn_mask).to(attn_mask.device) #all false, no mask

        return outputs_class, outputs_mask, attn_mask

    def process_mask(self, attn_mask, boundary):


        boundary_sig = (boundary.squeeze(dim=-2).sigmoid()>0.5).bool()
        bs, l = boundary_sig.shape
        attn_mask_refine = []
        for i in range(bs):
            boundary_pos = torch.nonzero(boundary_sig[i], as_tuple=False)
            for j in range():
                mask = attn_mask[i, :, :]
